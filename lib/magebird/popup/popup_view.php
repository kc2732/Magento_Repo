  ...
  {  
    ...
    foreach($collection as $popup): 
      ...
      if($popup['popup_type']==1){  
        if($popup['image'] && $popup['width_unit']==2){
          $imgSrc = urldecode($this->helper->getParam('baseUrl')) . "media/" . $popup['image'];          
        }elseif($popup['image']){
          $imgSrc = urldecode($this->helper->getParam('baseUrl')) . "media/" . $popup['image'];          
        }     
      ...
      $popup_js['totalSecondsDelay'] = $popup['total_seconds_delay'];
    ...
    var popupScriptListener = setInterval(function(){
      ...
      if(typeof jQuery !== 'undefined' && typeof mb_popup !== 'undefined' && !mb_popups){                                           
        ...
        mb_popup.doMousetracking = false<?php //echo $mousetracking;?>;
